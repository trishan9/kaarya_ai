...
class VapiMobileCall implements VapiCall {
  ...
  static Future<VapiMobileCall> create(
    ...
    // Parse the API response (use safe casts to handle nullable fields)
    final id = (apiResponse['id'] as String?) ?? '';
    final orgId = (apiResponse['orgId'] as String?) ?? '';
    final createdAt = DateTime.tryParse(
            (apiResponse['createdAt'] as String?) ?? '') ??
        DateTime.now();
    final updatedAt = DateTime.tryParse(
            (apiResponse['updatedAt'] as String?) ?? '') ??
        DateTime.now();
    final type = (apiResponse['type'] as String?) ?? 'webCall';
    final rawMonitor = apiResponse['monitor'];
    final monitor = rawMonitor is Map<String, dynamic>
        ? VapiCallMonitor.fromJson(rawMonitor)
        : const VapiCallMonitor(listenUrl: '', controlUrl: '');
    final rawTransport = apiResponse['transport'];
    final transport = rawTransport is Map<String, dynamic>
        ? VapiCallTransport.fromJson(rawTransport)
    ...
    final rawOverrides = apiResponse['assistantOverrides'];
    final assistantOverrides = rawOverrides is Map<String, dynamic>
        ? Map<String, dynamic>.from(rawOverrides)
        : rawOverrides is Map
            ? Map<String, dynamic>.from(
                rawOverrides.map((k, v) => MapEntry(k.toString(), v)))
        ...
        callStateUpdated: (stateData) {
          switch (stateData.state) {
            ...
            case CallState.joined:
              // Route audio to speakerphone once the call is joined.
              // Attempt immediately and again after a short delay, because
              // Daily may reset routing shortly after joining.
              try {
                _call.setAudioDevice(deviceId: DeviceId.speakerPhone);
              } catch (_) {}
